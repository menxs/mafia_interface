...
# Action, either accuse or night aciton
  ...
  def player_action(_phase, _player, _selected, %{alive: false}), do:
    ~E"""
    <td></td>
    """
  def player_action(:accusation, player, selected, _myself), do:
    accuse_button(player, selected)

  def player_action(:night, player, selected, myself), do:
    select_button(player, selected, myself)

  def player_action(_phase, _player, _selected, _myself), do:
    ~E"""
    <td></td>
    """
...
# Judgement Buttons (inno or guilty)
  ...
  def judgement_buttons(:judgement, %{name: name, alive: true}, accused, selected)
    when name != accused, do:
      ~E"""
      <%= innocent_button(selected) %>
      <%= guilty_button(selected) %>
      """
  def judgement_buttons(_, _, _, _), do:
    ~E"""
    """
...
# Help text
  ...
  def game_help(%{alive: false}), do:
    "You're dead, watch how the game unfolds."

  def game_help(%{name: name, role: :townie}), do:
    "You're #{name}, a member of the town, try to single out the mafiosos during the day and rest durig night."

  def game_help(%{name: name, role: :doctor}), do:
    "You're #{name} the doctor, during nights select someone to heal from a possible mafia attack."

  def game_help(%{name: name, role: :sheriff}), do:
    "You're #{name} the sheriff, during nights select someone to investigate their role."

  def game_help(%{name: name, role: :mafioso}), do:
    "You're #{name} the mafioso, During nights vote someone to kill as the mafia."

  def game_help(_myself), do:
    "You ain't got any help kiddo."
